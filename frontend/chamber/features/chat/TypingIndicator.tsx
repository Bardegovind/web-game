...
/** Rose, the colour between, and orchid: the love gradient, one dot at a time. */
const DOT_COLOURS = ['bg-lamp', 'bg-[#e25cce]', 'bg-violet'];
...
export function TypingIndicator({ name }: { name: string }) {
    ...
    return (
        <div data-typing-indicator="" role="status" aria-live="polite" className="flex w-full justify-start">
            ...
            <div className="bubble-theirs flex items-center gap-1.5 rounded-[var(--radius-bubble)] rounded-bl-sm px-4 py-4">
                {[0, 1, 2].map((i) => (
                    <motion.span
                        key={i}
                        aria-hidden="true"
                        className={`h-2 w-2 rounded-full ${DOT_COLOURS[i]}`}
                        animate={
                            reduceMotion
                                ? { opacity: [0.4, 1, 0.4] }
